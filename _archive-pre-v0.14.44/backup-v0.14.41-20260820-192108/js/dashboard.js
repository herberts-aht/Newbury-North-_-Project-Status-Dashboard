...
if(calendarMonthPicker){
  const selected=`${calendarYear}-${String(calendarMonth+1).padStart(2,"0")}`;
  if(!calendarMonthPicker.options.length){
    let options="";
    ...
    calendarMonthPicker.innerHTML=options;
  }
  calendarMonthPicker.value=selected;
}
